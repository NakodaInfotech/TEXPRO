...
Public Class PartyPackingSlip
    ...
    Public tempppsno As Integer     'used for poation no while editing
    ...
    Sub total()
        Try
            ...
            Next
            TXTBALMTRS.Text = Format(Val(txtpAppmtrs.Text) - Val(LBLTOTALMTRS.Text), "0.00")
    ...
    Sub getmaxno()
        ...
        DTTABLE = getmax(" isnull(max(PPS_no),0) + 1 ", "PARTYPACKINGSLIP", " AND PPS_cmpid=" & CmpId & " and PPS_locationid=" & Locationid & " and PPS_yearid=" & YearId)
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            For Each row As Windows.Forms.DataGridViewRow In GRIDPROGRAM.Rows
                If row.Cells(0).Value <> Nothing Then
                    If gridsrno = "" Then
                        ...
                        RECDATE = Format(DateTime.Parse(row.Cells(GDATE.Index).Value, New System.Globalization.CultureInfo("en-CA").DateTimeFormat), "MM/dd/yyyy")
                    ...
                    Else
                        ...
                        RECDATE = RECDATE & "," & Format(DateTime.Parse(row.Cells(GDATE.Index).Value, New System.Globalization.CultureInfo("en-CA").DateTimeFormat), "MM/dd/yyyy")
            ...
            Dim objCLSPARTYPACKINGSLIP As New ClsPARTYPACKINGSLIP()
            objCLSPARTYPACKINGSLIP.alParaval = alParaval
            If edit = False Then
                If USERADD = False Then
                    MsgBox("Insufficient Rights")
                    Exit Sub
                End If
                IntResult = objCLSPARTYPACKINGSLIP.save()
            ...
            ElseIf edit = True Then
                If USEREDIT = False Then
                    ...
                    Exit Sub
                End If
                alParaval.Add(tempppsno)

                IntResult = objCLSPARTYPACKINGSLIP.Update()
    ...
    Private Sub PPS_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If edit = True Then
                ...
                dttable = objCLSPARTYPACKINGSLIP.selectPARTYPACKINGSLIP(tempppsno, CmpId, Locationid, YearId)
                If dttable.Rows.Count > 0 Then

                    For Each dr As DataRow In dttable.Rows

                        txtpino.Text = tempppsno
                        Pidate.Value = Format(Convert.ToDateTime(dr("date")).Date, "dd/MM/yyyy")
                        ...
                        cmbitemname.Text = Convert.ToString(dr("MERCHANT").ToString)

                        'Item Grid
                        GRIDPROGRAM.Rows.Add(dr("GRIDSRNO").ToString, dr("LOTNO").ToString, dr("PIECETYPE").ToString, Format(dr("DATE"), "dd/MM/yyyy"), dr("BALENO").ToString, dr("MERCHANT").ToString, dr("QUALITY").ToString, dr("PROCESS").ToString, dr("JOBNO").ToString, dr("GRIDLRNO").ToString, dr("COLOR").ToString, dr("CUT").ToString, Format(dr("PCS"), "0.00"), Format(dr("MTRS"), "0.00"), Format(dr("OUTPCS"), "0.00"), Format(dr("OUTMTRS"), "0.00"), dr("FROMNO"), dr("FROMSRNO"), dr("FROMTYPE"), dr("LINO"), dr("LISRNO"))
                ...
                dttable = OBJCMN.search(" PPS_GRIDSRNO AS GRIDSRNO, PPS_REMARKS AS REMARKS, PPS_NAME AS NAME, PPS_IMGPATH AS IMGPATH, PPS_NEWIMGPATH AS NEWIMGPATH", "", " PARTYPACKINGSLIP_UPLOAD", " AND PPS_NO = " & tempppsno & " AND PPS_CMPID = " & CmpId & " AND PPS_LOCATIONID = " & Locationid & " AND PPS_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            'Dim objgrndetails As New PARTYPACKINGSLIPDetails
    ...
    Private Sub cmdselectPPS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdselectJO.Click
        Try
            ...
            OBJSELECTPO.FRMSTRING = "PARTYPACKINGSLIP"
            ...
            If selectpotable.Rows.Count > 0 Then
                ...
                For i = 0 To selectpotable.Rows.Count - 1
                    ...
                    CMBLOTNO.Text = selectpotable.Rows(i).Item("LOTNO")

                    GRIDPROGRAM.Rows.Add(i + 1, selectpotable.Rows(i).Item("LOTNO"), "", Format(Mydate.Date, "dd/MM/yyyy"), "", selectpotable.Rows(i).Item("ITEMNAME"), selectpotable.Rows(i).Item("QUALITY"), selectpotable.Rows(i).Item("PROCESS"), "", "", selectpotable.Rows(i).Item("COLOR"), selectpotable.Rows(i).Item("CUT"), Format(Val(selectpotable.Rows(i).Item("PCS")), "0.00"), Format(Val(selectpotable.Rows(i).Item("MTRS")), "0.00"), 0, 0, Val(selectpotable.Rows(i).Item("FROMNO")), Val(selectpotable.Rows(i).Item("FROMSRNO")), selectpotable.Rows(i).Item("FROMTYPE"), Val(selectpotable.Rows(i).Item("GRNNO")), Val(selectpotable.Rows(i).Item("GRNSRNO")))
    ...
    Private Sub tstxtbillno_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles tstxtbillno.Validating
        Try
            GRIDPROGRAM.RowCount = 0
            tempppsno = Val(tstxtbillno.Text)
            If tempppsno > 0 Then
                edit = True
                PPS_Load(sender, e)
            Else
    ...
    Private Sub toolnext_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles toolnext.Click
        Try
            ...
            getmaxno()
            clear()
            If Val(txtpino.Text) - 1 >= tempppsno Then
                edit = True
                PPS_Load(sender, e)
            Else
                clear()
    ...
    Sub fillgrid()
        ...
        If gridDoubleClick = False Then
            GRIDPROGRAM.Rows.Add(Val(txtsrno.Text.Trim), CMBLOTNO.Text.Trim, CMBPIECETYPE.Text.Trim, Format(Pidate.Value.Date, "dd/MM/yyyy"), TXTBALENO.Text.Trim, cmbitemname.Text.Trim, CMBQUALITY.Text.Trim, txtprocess.Text.Trim, TXTJOBNO.Text.Trim, txtlrno.Text.Trim, cmbcolor.Text.Trim, txtcutsize.Text, Format(Val(txtPCS.Text.Trim), "0.00"), Format(Val(TXTMTRS.Text.Trim), "0.00"), 0, 0, Val(txtfromno.Text), Val(txtfromsrno.Text), txtfromtype.Text.Trim, Val(txtgrnno.Text), Val(txtgrnsrno.Text), txtgrntype.Text)
            getsrno(GRIDPROGRAM)
        ElseIf gridDoubleClick = True Then
            ...
            GRIDPROGRAM.Item(GDATE.Index, temprow).Value = Format(Pidate.Value.Date, "dd/MM/yyyy")
            GRIDPROGRAM.Item(GMERCHANT.Index, temprow).Value = cmbitemname.Text.Trim
            GRIDPROGRAM.Item(GLOTNO.Index, temprow).Value = CMBLOTNO.Text.Trim
            GRIDPROGRAM.Item(GQUALITY.Index, temprow).Value = CMBQUALITY.Text.Trim
            GRIDPROGRAM.Item(GLRNO.Index, temprow).Value = txtlrno.Text.Trim
    ...
    Private Sub GRIDPROGRAM_CellDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDPROGRAM.CellDoubleClick
        If e.RowIndex >= 0 And GRIDPROGRAM.Item(GSRNO.Index, e.RowIndex).Value <> Nothing Then
            ...
            CMBPIECETYPE.Text = GRIDPROGRAM.Item(GPIECETYPE.Index, e.RowIndex).Value.ToString
            txtlrno.Text = GRIDPROGRAM.Item(GLRNO.Index, e.RowIndex).Value.ToString
            ...
            If CMBLOTNO.Text.Trim <> "" Then
                ...
                If dttable.Rows.Count > 0 Then
                    For i As Integer = 0 To dttable.Rows.Count - 1
                        ...
                        CMBQUALITY.Text = dttable.Rows(i).Item("QUALITY")
                        txtpAppmtrs.Text = dttable.Rows(i).Item("APPROVEDMTRS")
                        txtpApppcs.Text = dttable.Rows(i).Item("APPROVEDPCS")
                        CMBQUALITY.Text = dttable.Rows(i).Item("QUALITY")
    ...
    Private Sub CMBLOTNO_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBLOTNO.Validated
        If CMBLOTNO.Text <> "" Then CMBLOTNO.Enabled = False
    End Sub
    ...
    Private Sub PrintToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        ...
        End Try
    End Sub
    Sub printreport(ByVal initials As String)
        Try
            'Dim OBJCHALLAN As New commonLRNO
            'OBJCHALLAN.MdiParent = MDIMain
            'OBJCHALLAN.SELFORMULA = "{PPS.PPSNO}='" & initials & "' and {PPS.CMPID}=" & CmpId & " and {PPS.LOCATIONID}=" & Locationid & " and {PPS.YEARID}=" & YearId
            'OBJCHALLAN.FRMSTRING = "PPS"
            'OBJCHALLAN.Show()
