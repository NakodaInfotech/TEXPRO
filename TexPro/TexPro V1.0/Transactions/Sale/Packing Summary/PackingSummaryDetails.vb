...
Public Class PackingSummaryDetails
    ...
    Sub fillgrid(ByVal tepmcondition)
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" PACKINGSUMMARY.PS_no AS TEMPPACKNO, PACKINGSUMMARY.PS_date AS DATE, LEDGERS.Acc_cmpname AS NAME,  PACKINGSUMMARY.PS_REMARKS AS REMARKS, PACKINGSUMMARY.PS_LBLBALETOTALPCS AS PCS, PACKINGSUMMARY.PS_FINALMTR AS FINALMTR, PACKINGSUMMARY.PS_LONGATION AS LONGATION, ISNULL(PACKINGSUMMARY.PS_TOTALBALES,0) AS TOTALBALES, PACKINGSUMMARY.PS_SAMPLEMTR AS SAMPLEMTRS, PACKINGSUMMARY.PS_SHORTPCS AS SHORTPCS,PACKINGSUMMARY.PS_REJPCS AS REJPCS, PACKINGSUMMARY.PS_FENTMTRS AS FENTMTRS,PACKINGSUMMARY.PS_SHORTMTRS AS SHORTMTRS,PACKINGSUMMARY.PS_REJMTRS AS REJMTRS,   PACKINGSUMMARY.PS_GOODCUTMTRS AS GOODCUTMTRS,PACKINGSUMMARY.PS_RACKSMTRS RACKSMTRS ,PACKINGSUMMARY.PS_LESSPER AS LESSPER,  PACKINGSUMMARY.PS_LESSMTRS AS LESSMTRS, ISNULL(PACKINGSUMMARY.PS_CARTONCHGS,0) AS CARTONCHGS, ISNULL(PACKINGSUMMARY.PS_CUTTINGCHGS,0) AS CUTTINGCHGS, ISNULL(PACKINGSUMMARY.PS_FELT,0) AS FELT, ISNULL(PACKINGSUMMARY.PS_ZERO,0) AS ZERO", "", " PACKINGSUMMARY LEFT OUTER JOIN LEDGERS ON PACKINGSUMMARY.PS_LEDGERID = LEDGERS.Acc_id ", tepmcondition)
    ...
    Private Sub TOOLGRIDDTLS_Click(sender As Object, e As EventArgs) Handles TOOLGRIDDTLS.Click
        Try
            Dim OBJDTLS As New PackingSummaryGridDetails
