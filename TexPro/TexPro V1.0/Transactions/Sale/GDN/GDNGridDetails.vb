...
Public Class GDNGridDetails
    ...
    Sub FILLGRID()
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" GDN.GDN_no AS SRNO, GDN.GDN_date AS DATE, LEDGERS.Acc_cmpname AS CMPNAME, LEDGERS_1.Acc_cmpname AS AGENT, GDN.GDN_PONO AS PONO, GDN.GDN_DISPATCHDATE AS DISPATCHDATE, GDN.GDN_remarks AS REMARKS, ISNULL(TRANSLEDGERS.Acc_cmpname, '') AS TRANSNAME, ISNULL(LOCALTRANSLEDGERS.Acc_cmpname, '') AS LOCALTRANSNAME, ISNULL(GDN.GDN_EWAYBILLNO, '') AS EWBNO, GDN_PSDESC.GDN_PSNO AS BALENO, ITEMMASTER.item_name AS ITEMNAME, GDN_PSDESC.GDN_PCS AS PCS, GDN_PSDESC.GDN_MTRS AS MTRS, GDN_PSDESC.GDN_LRNO AS LRNO, ISNULL(GDN_PSDESC.GDN_BILLNO,'') AS BILLNO, ISNULL(PARTYLEDGERS.Acc_cmpname, '') AS PARTYNAME ", "", " GDN INNER JOIN GDN_PSDESC ON GDN.GDN_no = GDN_PSDESC.GDN_NO AND GDN.GDN_YEARID = GDN_PSDESC.GDN_YEARID INNER JOIN ITEMMASTER ON GDN_PSDESC.GDN_MERCHANTID = ITEMMASTER.item_id LEFT OUTER JOIN LEDGERS AS PARTYLEDGERS ON GDN_PSDESC.GDN_PARTYID = PARTYLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS ON GDN.GDN_ledgerid = LEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS LEDGERS_1 ON GDN.GDN_AGENTID = LEDGERS_1.Acc_id LEFT OUTER JOIN LEDGERS AS LOCALTRANSLEDGERS ON GDN.GDN_LOCALTRANSID = LOCALTRANSLEDGERS.Acc_id LEFT OUTER JOIN LEDGERS AS TRANSLEDGERS ON GDN.GDN_transid = TRANSLEDGERS.Acc_id ", " and dbo.gdn.gdn_yearid=" & YearId & " order by gdn.gdn_no, GDN_PSDESC.GDN_PSNO ")
