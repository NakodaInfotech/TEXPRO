    ...
    Private Sub TXTENTRYNO_Validated(sender As Object, e As EventArgs) Handles TXTENTRYNO.Validated
        Try
            If Val(TXTENTRYNO.Text.Trim) > 0 Then
                ...
                ElseIf CMBTYPE.Text = "JOBBALE" Then
                    DT = OBJCMN.search("ISNULL(ITEM_NAME,'') AS ITEMNAME", "", "PACKINGSLIP_JOB INNER JOIN ITEMMASTER ON PS_ITEMID = ITEM_ID ", " AND PS_NO = " & Val(TXTENTRYNO.Text.Trim) & " AND PS_YEARID = " & YearId)
                ElseIf CMBTYPE.Text = "OPJOBBALE" Then
                    DT = OBJCMN.search("ISNULL(ITEM_NAME,'') AS ITEMNAME", "", "STOCKMASTER INNER JOIN ITEMMASTER ON SM_ITEMID = ITEM_ID ", " AND SM_BALENO = " & Val(TXTENTRYNO.Text.Trim) & " AND SM_TYPE = 'JOBBALE' AND SM_YEARID = " & YearId)
                End If
    ...
    Private Sub CMDUPDATE_Click(sender As Object, e As EventArgs) Handles CMDUPDATE.Click
        Try
            If CMBITEMNAME.Text.Trim <> "" And (Val(TXTENTRYNO.Text.Trim) > 0 Or Val(TXTLOTNO.Text.Trim) > 0) And CMBTYPE.Text.Trim <> "" Then
                ...
                ElseIf CMBTYPE.Text = "JOBBALE" Then
                    DT = OBJCMN.Execute_Any_String(" UPDATE PACKINGSLIP_JOB SET PS_ITEMID = (SELECT ITEM_ID FROM ITEMMASTER WHERE ITEM_YEARID = " & YearId & " AND ITEM_NAME = '" & CMBITEMNAME.Text.Trim & "') WHERE PS_NO = " & Val(TXTENTRYNO.Text.Trim) & " AND PS_YEARID = " & YearId, "", "")
                    DT = OBJCMN.Execute_Any_String(" UPDATE PACKINGSLIP_DESC_JOB SET PS_ITEMID = (SELECT ITEM_ID FROM ITEMMASTER WHERE ITEM_YEARID = " & YearId & " AND ITEM_NAME = '" & CMBITEMNAME.Text.Trim & "') WHERE PS_NO = " & Val(TXTENTRYNO.Text.Trim) & " AND PS_YEARID = " & YearId, "", "")
                ElseIf CMBTYPE.Text = "OPJOBBALE" Then
                    DT = OBJCMN.Execute_Any_String(" UPDATE STOCKMASTER SET SM_ITEMID = (SELECT ITEM_ID FROM ITEMMASTER WHERE ITEM_YEARID = " & YearId & " AND ITEM_NAME = '" & CMBITEMNAME.Text.Trim & "') WHERE SM_BALENO = " & Val(TXTENTRYNO.Text.Trim) & " AND SM_TYPE= 'JOBBALE' AND SM_YEARID = " & YearId, "", "")
